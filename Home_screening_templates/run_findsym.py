...
def writeCIF(cell, prec, basename):
    ...
    f = open(('Output/' + basename + '_' + sgid + '.cif'), 'w')
...
if sys.argv[1][-4:] == '.cif':
    structure_name = sys.argv[1][:-4]
else:
    structure_name = sys.argv[1]

path_to_raspa_structure = os.path.join(os.environ['RASPA_DIR'],"share/raspa/structures/cif/")
path = os.path.join(path_to_raspa_structure,structure_name+".cif")

try:
    cell = ase.io.read(path)
except Exception as e:
    print('Could not read CIF structure from file: ' + path)
