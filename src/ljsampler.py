...
class load_coordinates():
    def __init__(self, molecules, forcefield = "UFF", temperature=298.0, cutoff=12.0):
    ...
    def sample(self, structure_name, unitcell, lattice_matrix):
        ...
        Args:
            ...
            lattice_matrix (numpy.2Darray): 3x3 array representing the unitcell vectors in the cartesian coordinate system of Raspa2 and Zeo++ (different from the one used in pymatgen)
        """
        ...
        for molecule in self.molecules:
            ...
            accessible_mean_energy.append(df_voro[df_voro["Energy_%s"%molecule]<0].mean())
            min_energy.append(df_voro[df_voro["Energy_%s"%molecule]<0].min())
        ...
        # TODO to_csv df_voro (create an output directory ?)
